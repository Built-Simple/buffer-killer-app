...
class GitHubAPI {
  ...
  async postStatus(content, options = {}) {
    ...
    try {
      ...
      const title = customTitle || content.substring(0, 50) + (content.length > 50 ? '...' : '');
      
      let result;
      
      if (type === 'issue') {
        // Post as an issue
        result = await this.createIssue(username, repo, title, content, ['social-post']);
      } else if (type === 'file') {
...
${content}`;
        ...
        result = await this.createRepoFile(username, repo, filename, fileContent, `Add post: ${title}`);
      } else if (type === 'gist') {
        // Post as a gist
        const filename = `${Date.now()}.md`;
        result = await this.createGist(description || title, {
